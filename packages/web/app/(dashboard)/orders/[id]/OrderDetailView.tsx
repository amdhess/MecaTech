...
export function OrderDetailView({ order }: OrderDetailViewProps) {
  ...
  const handleCopyLink = () => {
    ...
    toaster.create({
      title: "Link copiado!",
      description: "Envie para o cliente aprovar.",
      type: "success",
    });
  };
        ...
        <HStack gap={4}>
          ...
          <Stack gap={0}>
            <HStack>
              ...
              <RefreshButton size="xs" variant="outline" />
            </HStack>
          ...
          <Stack gap={4}>
            ...
            <HStack align="start" gap={3}>
              ...
              <Stack gap={0}>
                ...
                <Text color="fg.muted">{order.vehicle.client.name}</Text>
        ...
        >
          <Stack gap={4} justify="center">
            ...
            <Text color="fg.muted" fontSize="sm">
              O valor total desta ordem é baseado na soma das peças e serviços
              listados abaixo.
            </Text>
          </Stack>
        </Box>
          ...
          <Table.Root size="sm">
            ...
            <Table.Body>
              ...
              {(!order.services || order.services.length === 0) && (
                <Table.Row>
                  <Table.Cell colSpan={2} color="fg.muted" textAlign="center">
          ...
          <Table.Root size="sm">
            ...
            <Table.Body>
              {order.parts?.map((item) => (
                <Table.Row key={item.partId + item.serviceOrderId}>
                  <Table.Cell>
                    {item.part.name}{" "}
        ...
        >
          <Stack gap={3}>
            <HStack justify="space-between" color="fg.muted">
              ...
              <Text>{formatCurrency(order.totalServicesPrice ?? 0)}</Text>
            </HStack>
            <HStack justify="space-between" color="fg.muted">
              <Text>Total Peças</Text>
              <Text>{formatCurrency(order.totalPartsPrice ?? 0)}</Text>
            </HStack>
            <Separator />
            <HStack justify="space-between" fontWeight="bold" fontSize="lg">
              <Text>Total Geral</Text>
              <Text color="blue.600">{formatCurrency(order.totalPrice)}</Text>
            </HStack>
          </Stack>
        </Box>
      </Box>
